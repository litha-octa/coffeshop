...
function ManageOrderAdm(props) {
  const [modalShow, setModalShow] = useState(false);
  const [payment_method, setPayment_method] = useState("cash");
  useEffect(() => {
    console.log("test");
    props.getTransaction(
      `${process.env.REACT_APP_DOMAIN_API}:${process.env.REACT_APP_PORT_API}/transaction/bills`,
      props.token
    );
        ...
        <div className="container-payment">
          <div className="order-step">
            ...
            <div className="step-payment">
              ...
              <div id="text-header">Finish Order</div>
            </div>
          </div>
          <div id="text-checkout">Finish your customer order now</div>
          <div className="card-content-container">
            <div className="summary-card">
              <div id="text-summary">
                Dine in for<div id="text-normal"> Table </div>
              </div>
                    ...
                    {props.bills.results.result.subResult.map((items) => (
                      <div className="item-container" key={items.id}>
                        ...
                        <div className="item-desc">
                          <div className="item-name">{items.product_name}</div>
                          <div className="item-quantity">{items.quantity}x</div>
            ...
            <div className="user-order-card">
              ...
              <Modal
                ...
                btnlabel2="Confirm Done🤤"
              />
            </div>
          </div>
          <div className="slide-btn"> Slide to see upcoming orders </div>
        </div>
      )}
    </div>
  );
...
export default connect(mapStateToProps, mapDispatchToProps)(ManageOrderAdm);
